...
vector_store = VectorStore(Config.VECTOR_DB_PATH)   # initializing vector store object
storage_service = S3Storage()      # initializing storage service object
llm_service = LLMService(vector_store)    # initializing llm service object 
...
def upload_document():
    try:
        ...
        if 'file' not in request.files:  # checks if the file is there or not
            logger.warning("No file in request")
            return jsonify({'error': 'No file provided'}), 400
        
        file = request.files['file']  # checks if the file is empty or not
        if file.filename == '':
